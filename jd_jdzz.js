...
const inviteCodes = [
  `A3Y33SR4TGb2ydTm7ng@AUWE5ysnEs0lUfAa8nBsC@AUWE5mP6TmWdaWmPx1C5LlA`,
]
!(async () => {
  $.tuanList = []
  await requireConfig();
...
function readShareCode() {
  console.log(`开始`)
  return new Promise(async resolve => {
    $.get({url: `https://code.chiag.fun/api/v1/jd/jdzz/read/${randomCount}/`, 'timeout': 10000}, (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
          console.log(`${$.name} API请求失败，请检查网路重试`)
