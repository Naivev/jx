        ...
        if (err) {
          ...
          if (safeGet(data)) {
            ...
            if(data.code===0 && data.data.result){
              ...
              // void(0) // console.log(`您的助力码为${data.data.result.inviteCode}`)
...
function readShareCode() {
  ...
  return new Promise(async resolve => {
    $.get({url: `https://code.chang.fun/api/v1/jd/jdcash/read/${randomCount}/`, 'timeout': 10000}, (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
          console.log(`${$.name} API请求失败，请检查网路重试`)
...
function shareCodesFormat() {
  return new Promise(async resolve => {
    // void(0) // console.log(`第${$.index}个京东账号的助力码:::${$.shareCodesArr[$.index - 1]}`)
    $.newShareCodes = [];
    ...
    console.log(`第${$.index}个京东账号将要助力的好友${JSON.stringify($.newShareCodes).replace(/{[^}]+},/, '')}`)
    resolve();
  })
}
