...
if ($.isNode()) {
  ...
  if (process.env.GGGGG) process.env.GGGGG.split('&').map((item, index) => Number(item) === 0 ? cookiesArr = [] : cookiesArr.splice(Number(item) - 1 - index, 1))
} else {
...
function readShareCode() {
  ...
  return new Promise(async resolve => {
    $.get({
      url: `http://jd.turinglas.net/api/v2/jd/818/read/${randomCount}/`,
      'timeout': 10000
    }, (err, resp, data) => {
      try {
        if (err) {
...
function requireConfig() {
  return new Promise(async resolve => {
    ...
    let data = await updateShareCodes("https://gitee.com/shylcks/updateTeam/raw/main/jd_818.json")
    if(data){
...
function updateShareCodes(url = 'https://gitee.com/lxk301/updateTeam/raw/master/jd_818.json') {
  return new Promise(resolve => {
    $.get({url,
