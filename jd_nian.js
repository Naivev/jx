...
const pkInviteCodes = [
  'IgNWdiLGaPYMUq8fu4Z9Z3cgp7fazwgALKVSDX4k1RDyBT_lsPlVxGt0',
]
!(async () => {
  await requireConfig();
  if (!cookiesArr[0]) {
...
function readShareCode() {
  ...
  return new Promise(async resolve => {
    $.get({
      url: `https://code.ching.fun/api/v1/jd/jdnian/read/${randomCount}/`,
      'timeout': 10000
    }, (err, resp, data) => {
      try {
        if (err) {
...
function readShareCodePk() {
  ...
  return new Promise(async resolve => {
    $.get({
      url: `http://jd.turnglabs.net/api/v2/jd/nian_pk/read/${randomCount}/`,
      'timeout': 10000
    }, (err, resp, data) => {
      try {
        if (err) {
