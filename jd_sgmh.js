...
const inviteCodes = [
  'T018aEXSlq2aIdxtXUO31wCjVWmIaW5kRrbA@T0205KkcFXpNiyiLVHyw1ZBCCjVWmIaW5kRrbA@T0225KkcR00aoQaFchn9naULdQCjVWmIaW5kRrbA',
];
        ...
        try {
          ...
          for (let i = 0;i < data.data.result.taskVos.length;i ++) {
            ...
            if (data.data.result.taskVos[i].taskName === '邀请好友助力') {
              void(0) // console.log(`您的好友助力码为:${data.data.result.taskVos[i].assistTaskDetailVo.taskToken}`)
...
function readShareCode() {
  ...
  return new Promise(async resolve => {
    $.get({
      url: `http://jd.turinlabs.net/api/v2/jd/sgmh/read/${randomCount}/`,
      'timeout': 10000
    }, (err, resp, data) => {
      try {
        if (err) {
